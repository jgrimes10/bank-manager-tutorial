...
export class AccountsListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // add ViewChild support for the table MatPaginator

  private _accountsDataSource: MatTableDataSource<Account> = new MatTableDataSource<Account>();

  // add ViewChild support for the table column sorting
  ...
  get columns(): string[] {
    // returns a string array of the columns in the table
    // the order of these values will be the order your columns show up in
    return ['id', 'title', 'opened', 'currentBalance', 'info'];
  }

  ngOnInit(): void {
    // build the filter form group
    // add an entry for the user to enter filter text
    ...
    this.filterTableFormGroup.controls['filter'].valueChanges
      .pipe(
        debounceTime(1500), // wait 1.5 seconds for the user to finish entering info before applying filter
  ...
  ngAfterViewInit() {
    // register paginator & sort views children with the table data source
